...
const normalizePath=(value)=>{const path=new URL(value,window.location.origin).pathname.replace(/\/+$/,'');return path||'/';};
const currentPath=normalizePath(window.location.pathname);
const topLevelLinks=Array.from(nav.children).filter((item)=>item.matches('a:not(.nav-cta)'));
let activeItem=null;
if(currentPath.startsWith('/services')){activeItem=servicesDisclosure?servicesDisclosure.querySelector('summary'):null;}else if(currentPath.startsWith('/work/systems')||currentPath.startsWith('/work/hqcp')||currentPath.startsWith('/work/web-production-system')){activeItem=topLevelLinks.find((link)=>normalizePath(link.href)==='/work/systems');}else if(currentPath.startsWith('/work')){activeItem=topLevelLinks.find((link)=>normalizePath(link.href)==='/work');}else{activeItem=topLevelLinks.find((link)=>{const linkPath=normalizePath(link.href);return linkPath!=='/'&&currentPath.startsWith(linkPath);});}
if(activeItem){activeItem.classList.add('is-active');activeItem.setAttribute('aria-current',activeItem.matches('a')&&normalizePath(activeItem.href)===currentPath?'page':'location');}
nav.querySelectorAll('.services-menu a').forEach((link)=>{if(normalizePath(link.href)!==currentPath)return;link.classList.add('is-active');link.setAttribute('aria-current','page');});
const setPageScrollLock = (locked) => {
document.body.classList.toggle('mobile-nav-open', locked);
};
const syncServicesState = () => {
if (navShell && servicesDisclosure) {
navShell.classList.toggle('services-open', servicesDisclosure.open);
}
};
if (servicesDisclosure) {
servicesDisclosure.addEventListener('toggle', syncServicesState);
syncServicesState();
}
...
nav.classList.toggle('is-open', open);if(navShell)navShell.classList.toggle('mobile-nav-active',open&&window.innerWidth<=1220);
setPageScrollLock(open && window.innerWidth <= 1220);
if (!open) nav.style.removeProperty('--mobile-nav-top');
if (!open && servicesDisclosure) {
